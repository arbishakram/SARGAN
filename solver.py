...
class Solver(object):
    """Solver for training and testing SARGAN."""
    ...
    def test(self):
        """Translate images using USGAN."""

#        G_path = os.path.join(self.model_save_dir, '{}-G.ckpt'.format(resume_iters)) 
        G_path = os.path.join(self.model_save_dir, 'sargan_G.ckpt') 
        self.G.load_state_dict(torch.load(G_path, map_location=lambda storage, loc: storage))        
        
        data_loader = self.data_loader
        img_names = data_loader[1]
        ...
        with torch.no_grad():
            for i, (x_real, c_org) in enumerate(data_loader):
                ...
                exp_list = ['AFS', 'ANS', 'DIS', 'HAS', 'NES', 'SAS', 'SUS']
                imgn = img_names[i]
